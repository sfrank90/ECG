  ...
  if (file.is_open()) {
    while (file.good()) {
      ...
      if (!key.compare("f")) {
        ...
        while (sstr.good() && vCount < 4) {
          ...
          if (!sstr.good()) {
            // import of vertex index failed -> end of line reached //
			  //std::cout << "import failed " << ti[vCount];
            vi[vCount] = 0;
          } else {
		  //if (sstr.good()) {
            ...
            if (sstr.peek() == '/') {
              ...
              if (sstr.peek() != '/') {
                // there is a texture coordinate //
				  //std::cout << "tex " << ti[vCount];
                sstr >> ti[vCount];
              } else {
                ti[vCount] = 0;
				//std::cout << "no tex " << ti[vCount];
              }
              sstr.get(); // skip '/' symbol //
              sstr >> ni[vCount];
			  //std::cout << "ni " << ni[vCount];
            } else {
              ti[vCount] = 0;
              ni[vCount] = 0;
            }
		...
		//std::cout << lineNumber << " : " << vi[0] << " " << vi[1] << " " << vi[2] << std::endl;

		//if (localFace.size() > 50)
		//	return 0;
